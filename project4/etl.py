...
def process_song_data(spark, input_data, output_data):
    """ Process song data from local or remote file system """
    ...
    songs_table.write.partitionBy("year", "artist_id").parquet("{}/songs_table".format(output_data), mode="overwrite")
    ...
    artists_table.write.parquet(
        "{}/artists_table".format(output_data), mode="overwrite"
    )


def process_log_data(spark, input_data, output_data):
    """ Process log data from local or remote file system """
    ...
    df = df.withColumn(
        "timestamp",
        F.date_format(df.ts.cast(dataType=T.TimestampType()), "yyyy-MM-dd hh:mm:ss"),
    )
    df = df.withColumn(
        "datetime", F.date_format(df.ts.cast(dataType=T.TimestampType()), "yyyy-MM-dd")
    )
    # convert text to timestamp and datatype
    df = df.withColumn(
        "timestamp", F.to_date(df.timestamp.cast(dataType=T.TimestampType()))
    )
    ...
    df = df.withColumn(
        "weekday",
        F.when(
            (F.dayofweek(df.datetime) == 0)
            | (F.dayofweek(df.datetime) == 1)
            | (F.dayofweek(df.datetime) == 7),
            0,
        ).otherwise(1),
    )
    ...
    time_table.write.partitionBy("year", "month").parquet("{}/time_table".format(output_data), mode="overwrite")

    # read in song data to use for songplays table
    song_df = spark.read.option("basePath", "{}/songs_table".format(output_data)).parquet("{}/songs_table/*".format(output_data))
    ldf = df.alias("ldf")
    sdf = song_df.alias("sdf")
    tt = time_table.alias("tt")

    left_join_log_song = ldf.join(sdf, ldf.song == sdf.title, how="left")
    ljls = left_join_log_song.alias('ljls')

    #time_merge = ljls.join(tt, ljls.ts==tt.ts, how="left")

    # extract columns from joined song and log datasets to create songplays table
    #songplays_table = time_merge.select(
    songplays_table = left_join_log_song.select(
        "timestamp",
        "userId",
        "level",
        "song_id",
    ...
    songplays_table.write.partitionBy('year','month').parquet(
        "{}/songplays_table".format(output_data), mode="overwrite"
    )
